...
def wait_for_server(timeout_sec: float = 300.0) -> bool:
...
def multipart_body(fields: dict, file_field: str, file_path: Path) -> tuple[bytes, str]:
    ...
    boundary = '----trinetra-e2e-boundary-9f3a2c'
    lines = []
...
def main() -> int:
    ...
    try:
        ...
        try:
            ...
            print(f'  BODY: {exc.read().decode("utf-8")}\n', flush=True)
        except Exception as exc:
            failures.append(('POST /segment', exc))
            print(f'  FAIL: {exc}\n', flush=True)

        # 3. /predict is removed from production
        print('[test 3/3] POST /predict (per model) - SKIPPED (410 Gone expected)', flush=True)
        print('', flush=True)
